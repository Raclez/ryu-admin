  ...
  <div class="system-config-list">
            ...
            <template v-if="groupOptions.length > 0">
              ...
              >
                <div class="dict-group-option">
                  <el-tag :type="group.status === 1 ? 'success' : 'danger'" class="group-tag"
                          size="small">
        ...
        <el-form-item>
          <div class="action-buttons">
            <el-button :icon="Search" :loading="loading" type="primary"
                       @click="configStore.fetchConfigs()">查询
            </el-button>
            <el-button :disabled="loading" :icon="Refresh" @click="reset">重置</el-button>
            <el-button :disabled="loading" :icon="Plus" type="success" @click="addConfig">添加配置
            </el-button>
            <el-button
    ...
    <el-card v-loading="loading" class="table-card" element-loading-background="rgba(0, 0, 0, 0.1)"
        ...
        <div class="table-actions">
          ...
          <el-tooltip content="刷新数据" placement="top">
            <el-button :icon="Refresh" :loading="loading" circle text
                       type="primary" @click="configStore.fetchConfigs()"></el-button>
          </el-tooltip>
          <el-tooltip content="导出数据" placement="top">
            <el-button :icon="Download" circle text type="primary" @click="exportData"></el-button>
          </el-tooltip>
        </div>
      </div>
      ...
      <el-table
        ...
        <el-table-column align="center" label="配置值" min-width="120" prop="configValue"
                         show-overflow-tooltip/>
        <el-table-column :visible="!isMobile" align="center" label="配置分组" min-width="100"
                         prop="configGroup">
          <template #default="scope">
            <el-tag :type="getGroupTagType(getDictGroupName(scope.row.dictId))" size="small">
        ...
        <el-table-column :visible="!isMobile" align="center" label="配置备注" min-width="120"
                         prop="remark" show-overflow-tooltip/>


        <el-table-column align="center" fixed="right" label="扩展信息" width="160">
          <template #default="scope">
            <el-button
              class="expand-button"
              link
        ...
        <el-table-column :visible="!isMobile" align="center" label="创建时间" min-width="180">
          <template #default="scope">
            {{ formatDateTime(scope.row.createTime) }}
          </template>
        </el-table-column>

        <el-table-column :visible="!isMobile" align="center" label="修改时间" min-width="180">
          <template #default="scope">
      ...
      <div v-if="configStore.configList.length === 0 && !loading" class="empty-data">
    ...
    >
            ...
            <el-col :span="24">
              <el-form-item label="配置值" prop="configValue" required>
                <el-input
                  ...
                  :type="isJsonValue(currentConfig.configValue) ? 'textarea' : 'text'"
                  class="full-width-input"
                  placeholder="请输入配置值"
                  show-word-limit
                />
                <div v-if="isJsonValue(currentConfig.configValue)" class="input-tip">
                  <el-icon>
                    <Warning/>
                  </el-icon>
          ...
          <el-row :gutter="24">
            ...
            <el-col :span="12">
              <el-form-item label="状态" prop="status">
                <el-radio-group v-model="currentConfig.status" class="status-radio-group">
                  <el-radio :label="1">
                  ...
                  <el-tab-pane label="结构化编辑">
                    <div class="extra-structured-editor">
                      ...
                      <div class="extra-actions">
                        <el-button :icon="Plus" size="small" type="primary" @click="addExtraField">
                          添加字段
                        </el-button>
                      </div>
                    </div>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button :disabled="submitting" class="cancel-button" @click="dialogVisible = false">
            取消
          </el-button>
          <el-button :loading="submitting" class="submit-button" type="primary" @click="submitForm">
            确认
          </el-button>
        </div>
      </template>
...
const fetchConfigGroups = async () => {
  try {
    ...
    groupOptions.value = await getDictItemsByContidion(params);
    console.log('获取配置分组成功', groupOptions.value);
  } catch (error) {
...
const exportData = () => {
  const headers = [
    '配置键', '配置值', '配置分组', '状态', '备注', '扩展信息', '创建时间', '更新时间'
  ];
